...
export default function Settings() {
  ...
  const [importPassword, setImportPassword] = useState(''); // For Import
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [, setImportFile] = useState<any>(null);
  ...
  const toggleLanguage = async () => {
    ...
    // Force re-render if necessary, though useTranslation hook should handle it
  };
  ...
  const handleDecrypt = () => {
    ...
    try {
      ...
      const bytes = CryptoJS.AES.decrypt(parsed.content, importPassword);
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20">
      ...
      <section className="glass-card p-6 rounded-2xl space-y-4">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm text-slate-500 mb-1">Export Encryption Password (Optional)</label>
            <input
              type="password"
              placeholder="Set a password for the export..."
